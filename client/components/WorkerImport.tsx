...
export const WorkerImport: React.FC<WorkerImportProps> = ({
  ...
  const validateWorkerData = (data: any, rowIndex: number): ImportedWorker => {
    ...
    // بعد ما تجيب بيانات worker (و data هي مصدر البيانات الأصلية لكل worker)
if (worker.nom) {
  if (!worker.eponge || worker.eponge.trim() === 'non') {
    // لو eponge غير موجودة أو فارغة، نعطيها القيمة الافتراضية "non"
    worker.eponge = 'oui';
  }
}
    const result = {
